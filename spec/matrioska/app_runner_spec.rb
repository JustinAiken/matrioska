...
module Matrioska
  describe AppRunner do
    ...
    describe "#stop!" do
      let(:mock_componenet) { double Punchblock::Component::Input, register_event_handler: true }

      before do
        Punchblock::Component::Input.stub(:new).and_return mock_componenet
        call.stub(:write_and_await_response)
        subject.start
      end

      it "stops the runner" do
        mock_componenet.should_receive(:executing?).and_return true
        mock_componenet.should_receive :stop!
        subject.stop!
